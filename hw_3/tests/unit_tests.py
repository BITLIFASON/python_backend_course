...
def test_get_class(text, label):
    class_pred = requests.post('http://localhost:8003/classification/', json={"text": text}).json()
...
def test_get_sentiment(text, label):
    sent_pred = requests.post('http://localhost:8002/sentiment/', json={"text": text}).json()
