...
async def get_analysis(text: str) -> str:
    ...
    """
    sent_text = requests.post(
        "http://sentiment_service:8002/sentiment/", json={"text": text}
    ).json()  # http://127.0.0.1:8002/sentiment/
    class_text = requests.post(
        "http://classification_service:8003/classification/", json={"text": text}
    ).json()  # http://127.0.0.1:8003/classification/
